...
const Navbar = () => {
  ...
  return (
    <header className="fixed w-full top-0 z-50 bg-transparent mt-3">
        ...
        <div className="flex items-center justify-between h-full">
          ...
          >
            <Link to={"/settings"} className="btn btn-sm gap-2">
              <Settings className="w-6 text-white h-8 hover:scale-105 ease-linear duration-150" />
            </Link>

            {authUser && (
              <>
                <Link to={"/profile"} className="btn btn-sm gap-2">
                  <User className="w-6 h-8 text-white hover:scale-105 ease-linear duration-150" />
                </Link>
